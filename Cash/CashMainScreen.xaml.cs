...
namespace Cash
{
    ...
    public partial class CashMainScreen :ScreenBase ,IScreen, IPreBindable
    {
        ...
        {
            //RES 5/21/15 Do not process save if batch is posted
            if (chkPosted.IsChecked == 1)
            {
                MessageBox.Show("Save is disabled because batch has been posted!");
                SaveSuccessful = false;
                this.Load();
                return;
            }
            //RES 5/27/15 do not let user delete alloc rows if the batch has already been saved
            if (CurrentState != ScreenState.Deleting)
                foreach (DataRow r in CurrentBusObj.ObjectData.Tables["remit_alloc"].Rows)
                {
                    if (r.RowState == DataRowState.Deleted && Convert.ToInt32(CurrentBusObj.ObjectData.Tables["cash_batch"].Rows[0]["batch_id"]) > 0)
                    {
                        MessageBox.Show("Cannot delete remit allocation on a batch that has been saved!");
                        SaveSuccessful = false;
                        this.Load();
                        return;
                    }
            ...
            //RES 3/10/15 put in below test for deleting to bypass check for batch total 
            if (CurrentState != ScreenState.Deleting)
            {
                if (tBatchTotal.Text.ToString() == "$0.00")
                {  
                    MessageBox.Show("Batch not saved. Batch amount must be greater than 0.");
                    return;
                }
            }

            //RES 7/12/19 Check that cash batch date is within 3 days of current date
            if (CurrentState != ScreenState.Deleting)
            {
                if ((Convert.ToDateTime(CurrentBusObj.ObjectData.Tables["cash_batch"].Rows[0]["batch_date"]) < DateTime.Now.AddDays(-4)) ||
                    (Convert.ToDateTime(CurrentBusObj.ObjectData.Tables["cash_batch"].Rows[0]["batch_date"]) > DateTime.Now))
                {
                    var result = MessageBox.Show("Batch date is not within 3 days of current date.  Do you want to proceed? ", "BATCH DATE WARNING", MessageBoxButton.YesNo);
                ...
                //RES 7/16/19 Check that cash batch date and remit dates are the same
                foreach (DataRow dr in this.CurrentBusObj.ObjectData.Tables["remit"].Rows)
                {
                    if (dr["remit_date"].ToString() != dpBatchDate.SelText.ToString())
                    {
                        dr["remit_date"] = dpBatchDate.SelText.ToString();
                    }
            ...
            if (SaveSuccessful)
            {
                if (CurrentState == ScreenState.Deleting)
                {
                    ...
                    tBatchID.CntrlFocus();
                    tBatchID.Text = "";
                    //RES 3/10/15 close window after delete so that user does not try to enter data in empty screen
                    //this.Close();
                }
                else
                {
        ...
        /// The delete works by deleting all of the rows in the appropriate tables being deleted from the dataset
        /// Then doing a Save()
        /// </summary>
        /// 
        ...
        public override void Close()
        {
                ...
                && (this.SecurityContext == AccessLevel.ViewUpdate || this.SecurityContext == AccessLevel.ViewUpdateDelete))
            {
                ...
                if (result == MessageBoxResult.OK || result == MessageBoxResult.Yes)
                {
                    ...
                    {
                        MessageBox.Show("Batch not saved. Amount must be greater than 0.");
                        return;
                    }
        ...
        public override void Delete()
        {
            ...
            else //Not a posted batch
            {
                ...
                if (result == System.Windows.MessageBoxResult.Yes)
                {
                    ...
                    //RES 3/10/15 uncommented below statement because save needs to know a delete is being performed
                    ...
                    if (CurrentBusObj.ObjectData.Tables["remit_alloc"] != null && CurrentBusObj.ObjectData.Tables["remit_alloc"].Rows.Count > 0)
                        CurrentBusObj.ObjectData.Tables["remit_alloc"].AcceptChanges();
                    ...
                    if (CurrentBusObj.ObjectData.Tables["remit"] != null && CurrentBusObj.ObjectData.Tables["remit"].Rows.Count > 0)
                        CurrentBusObj.ObjectData.Tables["remit"].AcceptChanges();
                        CurrentBusObj.DeleteAllTableRows("remit");

                    if (CurrentBusObj.ObjectData.Tables["cash_batch"] != null && CurrentBusObj.ObjectData.Tables["cash_batch"].Rows.Count > 0)
                        CurrentBusObj.ObjectData.Tables["cash_batch"].AcceptChanges();
                        CurrentBusObj.DeleteAllTableRows("cash_batch");
                    
                    //Run the save to delete all rows from database if the cash batch has already been saved.
                    ...
                    if (CurrentState == ScreenState.Inserting)
                    {
                        ...
                        dtMiscInfo.Rows[0]["total_allocated"] = 0;
                        dtMiscInfo.Rows[0]["total_remit"] = 0;

                        tRemitTotal.DataContext = dtMiscInfo;
                        tAllocTotal.DataContext = dtMiscInfo;
                        tRemitLineTotal.DataContext = dtMiscInfo;
                        tBatchID.Text = " ";
                        //tRemitLineTotal.Text = "0.00";
                        //tRemitTotal.Text = "0.00";
                        //tAllocTotal.Text = "0.00";
                        CurrentState = ScreenState.Normal;
                        Messages.ShowInformation("Batch cleared from screen.");
                        return;
        ...
        private bool VerifyData()
        {
            ...
            foreach (DataRow r in CurrentBusObj.ObjectData.Tables["remit_alloc"].Rows)
            {
                //RES 10/15/19 skip deleted rows, changing an amount to 0 on a saved batch was causing it to bomb
                ...
                //{
                    ...
                    else //Applied amount
                    {
                        ...
                        //3) more than open amount was not allocated
                        if (Convert.ToDecimal(r["amount"]) < 0)
                            s += "Allocated amount must be greater than 0. ";
                        ...
                        //RES 5/21/15 Make sure amount allocated does not exceed open amount
                        if (Convert.ToDecimal(r["amount"]) > Convert.ToDecimal(r["open_amount"]) && Convert.ToDecimal(r["open_amount"]) > 0)
                            s += "Allocated amount " + Convert.ToDecimal(r["amount"]).ToString() + " exceeds " + r["apply_to_doc"].ToString() + " open amount " +
                                  Convert.ToDecimal(r["open_amount"]).ToString("N2");
                    }

                    //Verify Currency codes match between remit and remit alloc document selected
                    //RES 5/22/15 added && RemitTable.RowState != DataRowState.Deleted
                    EnumerableRowCollection<DataRow> RemitRow = from RemitTable in CurrentBusObj.ObjectData.Tables["remit"].AsEnumerable()
                                                                ...
                                                                select RemitTable;
                                                                //where RemitTable.RowState != DataRowState.Deleted
                                                               //&& RemitTable.Field<Int32>("remit_id") == Convert.ToInt32(r["remit_id"])
                                                               //select RemitTable;

                    foreach (DataRow remit in RemitRow)
                    {
                        //if (remit.RowState != DataRowState.Deleted)
        ...
        //private void dpBatchDate_LostFocus(object sender, RoutedEventArgs e)
        //{
        //    //RES 7/12/19 Check that cash batch date entered is within 3 days of current date           
        //    if ((dpBatchDate.SelText < DateTime.Now.AddDays(-3)) || (dpBatchDate.SelText > DateTime.Now))            
        //        MessageBox.Show("Batch date is not within 3 days of current date.");             
        //}
        ...
        private void CalculateAmountAllocated()
        {
            ...
            if (CurrentBusObj.ObjectData.Tables["remit"] != null && CurrentBusObj.ObjectData.Tables["remit"].Rows.Count > 0)
            {
                ...
                {                    
                    if (r.RowState == DataRowState.Deleted && Convert.ToInt32(CurrentBusObj.ObjectData.Tables["cash_batch"].Rows[0]["batch_id"]) > 0)
                    {
                        MessageBox.Show("Cannot delete remit on a batch that has been saved!");
                        SaveSuccessful = false;
                        this.Load();
                        return;
                    }
        ...
        private string CompareTotals()
        {
            ...
            {
                //if (rRemit.RowState != DataRowState.Deleted)
                //{
        ...
        {
            //RES 3/11/15 Get rid of empty allocation rows
            GridEmptyRowsProcessing(gAlloc, true);
        ...
        private void CreateNewAllocRow()
        {
            ...
            else
            {
                ...
                //RES 5/21/15 add open amount to allocation grid to validate that too much cash does not get allocated to a document
                row.Cells["open_amount"].Value = 0.0;
        ...
        public void gRemitGrid_EditModeEnded(object sender, Infragistics.Windows.DataPresenter.Events.EditModeEndedEventArgs e)
        {
            ...
            if (e.Cell.Field.Name == "amount_functional")
            {
                ...
                decimal dAmountFunc = 0.0M;

                if (!decimal.TryParse(e.Cell.Value.ToString(), out dAmountFunc))
                {
                    MessageBox.Show("Remit functional amount invalid.");
                    gRemit.Focus();
                    return;
                }
        ...
        public void gAllocGrid_EditModeEnded(object sender, Infragistics.Windows.DataPresenter.Events.EditModeEndedEventArgs e)
        {
            ...
            {
                decimal dAmountAlloc = 0.00M;
                if (!Decimal.TryParse(gAlloc.xGrid.ActiveCell.Value.ToString(), out dAmountAlloc))
                {
                    MessageBox.Show("Remit allocation amount is invalid.");
                    gAlloc.Focus();
                    return;
            ...
            if (e.Cell.Field.Name == "unapplied_flag")
            {
                if (Convert.ToInt32(e.Cell.Value) == 0) //Unapplied not checked
                {
                    ...
                    //RES 3/11/15 Do not blank out fields if when changing the amount on an allocation line that is not UA cash
                    if (gAlloc.ActiveRecord.Cells["apply_to_doc"].Value.ToString() == string.Empty)
                    {
                ...
                else //Unapplied is checked - User must enter product and customer - Amount defaults to remianing open
                {
                    ...
                    //RES 10/24/19 Only if on a line that is unapplied
                    if (gAlloc.ActiveRecord.Cells["unapplied_flag"].Value.ToString() == "1")
                    {
            ...
            if (e.Cell.Field.Name == "apply_to_doc")
            {
                ...
                if (CurrentBusObj.ObjectData.Tables["open_cash"] != null && CurrentBusObj.ObjectData.Tables["open_cash"].Rows.Count > 0)
                {
                    ...
                    else  //Single apply to row
                    {
                        ...
                        //RES 5/21/15 save open amount to be compared with amount to make sure too much cash is not allocated
                        gAlloc.ActiveRecord.Cells["open_amount"].Value = CurrentBusObj.ObjectData.Tables["open_cash"].Rows[0]["open_amount"];
            ...
            if (e.Cell.Field.Name == "apply_to_seq")
            {
                ...
                foreach (DataRow r in DocRow)
                {
                    ...
                    //RES 5/21/15 save open amount to be compared with amount to make sure too much cash is not allocated
                    gAlloc.ActiveRecord.Cells["open_amount"].Value = r["open_amount"];
