...
{
    /// <summary>
    /// This class represents a 'AdjustmentReverseInvoice' object.
    /// </summary>
    public partial class AdjustmentReverse : ScreenBase, IPreBindable
    {
        public cBaseBusObject ReverseAdjustmentBusObj = new cBaseBusObject();
        public AdjustmentFolder AdjFolderScreen;
        public Int32 AdjType = 17;
        ...
        /// </summary>
        public AdjustmentReverse(AdjustmentFolder _AdjFolderScreen)
            : base()
        {
            //set obj
            this.CurrentBusObj = ReverseAdjustmentBusObj;
            //name obj
            this.CurrentBusObj.BusObjectName = "AdjustmentReverse";
        ...
        private void SetParentChildAttributes()
        {
            ...
            GridReverseSource.xGrid.FieldLayoutSettings = f;
            GridReverseDestination.xGrid.FieldLayoutSettings = f;
            GridReverseSource.xGrid.EditModeEnded += new EventHandler<Infragistics.Windows.DataPresenter.Events.EditModeEndedEventArgs>(xGrid_EditModeEnded);
            GridReverseSource.MainTableName = "main";
            GridReverseSource.ConfigFileName = "AdjustmentGridReverseSource";
            GridReverseSource.FieldLayoutResourceString = "GridReverseSource";
            //Set the grid to allow edits, for readonly columns set the allowedit to false in the field layouts file
            GridReverseSource.xGrid.FieldSettings.AllowEdit = true;
            GridReverseSource.SetGridSelectionBehavior(true, false);
            //GridReverseSource.IsEnabled = false;
            GridReverseSource.SkipReadOnlyCellsOnTab = true;
            GridReverseSource.CellUpdatedDelegate = GridSource_CellUpdated;
            GridReverseSource.ChildSupport.Add(new ChildSupport() { ChildFilterOnColumnNames = { "product_code" }, ChildGrids = { GridReverseDestination }, ParentFilterOnColumnNames = { "product_code" } });

            //setup attributes for Child
            GridReverseDestination.xGrid.FieldLayoutSettings.HighlightAlternateRecords = true;
            GridReverseDestination.xGrid.FieldLayoutSettings.SelectionTypeField = Infragistics.Windows.Controls.SelectionType.Single;
            GridReverseDestination.MainTableName = "detail";
            GridReverseDestination.ConfigFileName = "AdjustmentGridReverseDestination";
            GridReverseDestination.FieldLayoutResourceString = "GridReverseDestination";
            GridReverseDestination.SetGridSelectionBehavior(false, false);
            GridReverseDestination.xGrid.FieldSettings.AllowEdit = false;
            GridReverseDestination.IsEnabled = false;
            GridReverseDestination.SkipReadOnlyCellsOnTab = true;
            GridCollection.Add(GridReverseSource);
            GridCollection.Add(GridReverseDestination);
        }
        ...
        {
            int edit_index = GridReverseSource.ActiveRecord.Cells.IndexOf(e.Cell);
            ...
            GridReverseSource.xGrid.ExecuteCommand(DataPresenterCommands.CommitChangesToActiveRecord);
            DataRecord GridRecord = null;
            GridRecord = GridReverseSource.ActiveRecord;
            ...
            if (GridRecord != null)
            {
                ...
                foreach (DataRecord r in GridReverseSource.xGrid.Records)
                {
                    //sum adj amts
        ...
        public override void Save()
        {
            ...
            if (DefPoolError)
            {
                Messages.ShowError("Cannot reverse normalized line before Def Pool Job has run.  Run Def Pool Job then create adjustment.");
                return;
            }
            MessageBoxResult result1;
            if (Normalized)
            {
                result1 = Messages.ShowYesNo("Reversing current invoice will cause the following invoice(s) to be reversed: " +
        ...
        private System.Collections.Generic.List<string> getValsFromSourceGrid()
        {
            ...
            GridReverseSource.ReturnSelectedData(SourceGridList);
        ...
        private void popGrid()
        {
            ...
            if (this.CurrentBusObj.ObjectData.Tables["main"].Rows.Count > 0)
            {
                ...
                if (Convert.ToInt32(this.CurrentBusObj.ObjectData.Tables["main"].Rows[0]["wht_amount"]) != 0)
                {
                    ...
                    chkReverseAll.IsEnabled = true;
                    GridReverseSource.IsEnabled = true;
                }
                else
                {
                    chkReverseAll.IsEnabled = true;
        ...
        private void setInitScreenState()
        {
            ...
            GridReverseDestination.IsEnabled = true;
        ...
        private void totGridRecs()
        {
            ...
            //foreach (DataRecord r in GridReverseDestination.xGrid.Records)
            //{
        ...
        private void GridSource_CellUpdated(object sender, Infragistics.Windows.DataPresenter.Events.CellUpdatedEventArgs e)
        {
            ...
            if (e.Cell.Field.Name == "rebill_flag")
            {
                DataRecord GridRecord = null;
                //GridReverseSource.xGrid.ExecuteCommand(DataPresenterCommands.CommitChangesToActiveRecord);
                GridReverseSource.xGrid.ExecuteCommand(DataPresenterCommands.CommitChangesToAllRecords);
                //RES 12/29/14 fixed issues when using reverse all on invoices with multiple product codes
                //GridRecord = GridReverseSource.ActiveRecord;
                ...
                if (GridRecord != null)
                {
                    ...
                    {
                        //RES add any debit adjustments to the invoice/recv doc line amount to get the amount of the adjustment
        ...
        private void chkReverseAll_Checked(object sender, System.Windows.RoutedEventArgs e)
        {
        ...
        private void chkReverseAll_UnChecked(object sender, System.Windows.RoutedEventArgs e)
        {
