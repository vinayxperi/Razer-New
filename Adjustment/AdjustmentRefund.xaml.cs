...
{
    /// <summary>
    /// This class represents a 'AdjustmentRefund' object.
    /// </summary>
    public partial class AdjustmentRefund : ScreenBase
    {
        public cBaseBusObject AdjRefundBusObject = new cBaseBusObject();
        public AdjustmentFolder AdjFolderScreen;
        public int refundType ;
        public string refundTypeValue;
        ...
        /// </summary>
        public AdjustmentRefund(AdjustmentFolder _AdjFolderScreen)
            : base()
        {
            //set obj
            this.CurrentBusObj = AdjRefundBusObject;
            //name obj
            this.CurrentBusObj.BusObjectName = "AdjustmentRefund";
        ...
        public void Init()
        {
            ...
            //setup adj grid
            GridRefund.xGrid.EditModeEnded += new EventHandler<Infragistics.Windows.DataPresenter.Events.EditModeEndedEventArgs>(xGrid_EditModeEnded);
            GridRefund.xGrid.FieldLayoutSettings.HighlightAlternateRecords = true;
            GridRefund.xGrid.FieldLayoutSettings.SelectionTypeField = Infragistics.Windows.Controls.SelectionType.Single;
            GridRefund.SkipReadOnlyCellsOnTab = true;
            GridRefund.ContextMenuAddIsVisible = false;
            GridRefund.ContextMenuRemoveIsVisible = false;
            GridRefund.MainTableName = "main";
            GridRefund.ConfigFileName = "AdjustmentRefund";
            GridRefund.SetGridSelectionBehavior(false, false);
            GridRefund.xGrid.FieldSettings.AllowEdit = true;
            GridRefund.IsEnabled = false;
            GridRefund.FieldLayoutResourceString = "GridRefund";
            GridCollection.Add(GridRefund);
            loadParms();
            this.Load();
        ...
        {
            int edit_index = GridRefund.ActiveRecord.Cells.IndexOf(e.Cell);
            ...
            if (edit_index == 7) //Amount To Adjust Field
            {
                ...
                GridRefund.xGrid.ExecuteCommand(DataPresenterCommands.CommitChangesToActiveRecord);
                DataRecord GridRecord = null;
                GridRecord = GridRefund.ActiveRecord;
                ...
                if (GridRecord != null)
                {
                    ...
                    foreach (DataRecord r in GridRefund.xGrid.Records)
                    {
                        //sum adj amts
      ...
      
        {
            if (this.CurrentBusObj.ObjectData != null)
                if ((e.Key == Key.Tab || e.Key == Key.Enter) && this.CurrentBusObj.ObjectData.Tables["main"].Rows.Count > 0)
                {
                    ...
                    {
                        setEditScreenState();
                        GridRefund.xGrid.ExecuteCommand(DataPresenterCommands.StartEditMode);
                        e.Handled = true;
                    }
                }
        }
        ...
        private void txtAdjAmt_LostFocus(object sender, System.Windows.RoutedEventArgs e)
        {
            ...
            if (Convert.ToDouble(txtAdjAmt.Text) <= 0)
                {
                    Messages.ShowInformation("Refund Amount must be positive");
                    txtCustomer.CntrlFocus();
                    return;
                }
        ...
        {
            //Enable fields
              GridRefund.IsEnabled = true;
           
            //Set the amount_adjusted field as active
            if (this.CurrentBusObj.ObjectData != null)
                if (this.CurrentBusObj.ObjectData.Tables["main"].Rows.Count > 0)
                    (GridRefund.xGrid.Records[GridRefund.ActiveRecord.Index] as DataRecord).Cells["amount_to_adjust"].IsActive = true;
        }
        ...
        public override void Save()
        {
            ...
            if (AmountsOffset())
            {
                ...
                //change to a type = 12 for Refund adjustment

                string NewDocID = cGlobals.BillService.InsertNewAdjustmentPreamble(DocumentID.ToString(), 12, cGlobals.UserName.ToLower(), Convert.ToDecimal(sAdjAmt));
