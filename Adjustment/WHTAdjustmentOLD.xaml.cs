...
namespace Adjustment
{
    ...
    /// </summary>
    public partial class WHTAdjustment : ScreenBase
    {
        public cBaseBusObject AdjWHTBusObject = new cBaseBusObject();
        public AdjustmentFolder AdjFolderScreen;
        public int whtType ;
        public string whtTypeValue;
        ...
        /// </summary>
        public WHTAdjustment(AdjustmentFolder _AdjFolderScreen)
            : base()
        {
            //set obj
            this.CurrentBusObj = AdjWHTBusObject;
            //name obj
        ...
        public void Init()
        {
            ...
            //setup adj grid
            GridWHT.xGrid.EditModeEnded += new EventHandler<Infragistics.Windows.DataPresenter.Events.EditModeEndedEventArgs>(xGrid_EditModeEnded);
            GridWHT.xGrid.FieldLayoutSettings.HighlightAlternateRecords = true;
            GridWHT.xGrid.FieldLayoutSettings.SelectionTypeField = Infragistics.Windows.Controls.SelectionType.Single;
            GridWHT.SkipReadOnlyCellsOnTab = true;
            GridWHT.ContextMenuAddIsVisible = false;
            GridWHT.ContextMenuRemoveIsVisible = false;
            GridWHT.MainTableName = "main";
            GridWHT.ConfigFileName = "AdjustmentWHTDocument";
            GridWHT.SetGridSelectionBehavior(false, false);
            GridWHT.xGrid.FieldSettings.AllowEdit = true;
            GridWHT.IsEnabled = false;
            GridWHT.FieldLayoutResourceString = "GridWHT";
            GridCollection.Add(GridWHT);
            loadParms();
            this.Load();
        ...
        {
            int edit_index = GridWHT.ActiveRecord.Cells.IndexOf(e.Cell);

          

            //WARNING: You may need to change this if the order of the AdjustmentFieldLayouts\GridConversionInvoice changes
            if (edit_index == 6) //Amount To Adjust Field
            {
                //commit user entered value to datatable
                GridWHT.xGrid.ExecuteCommand(DataPresenterCommands.CommitChangesToActiveRecord);
                DataRecord GridRecord = null;
                GridRecord = GridWHT.ActiveRecord;
                ...
                if (GridRecord != null)
                {
                    ...
                    foreach (DataRecord r in GridWHT.xGrid.Records)
                    {
                        //sum adj amts
      ...
      
        {
            if (this.CurrentBusObj.ObjectData != null)
                if ((e.Key == Key.Tab || e.Key == Key.Enter) && this.CurrentBusObj.ObjectData.Tables["main"].Rows.Count > 0)
                {
                    ...
                    {
                        setEditScreenState();
                        GridWHT.xGrid.ExecuteCommand(DataPresenterCommands.StartEditMode);
                        e.Handled = true;
                    }
                }
        }
        ...
        private void txtAdjAmt_LostFocus(object sender, System.Windows.RoutedEventArgs e)
        {
            ...
            if (whtType == 0)
                if (Convert.ToDouble(txtAdjAmt.Text) >= 0)
                {
                    Messages.ShowInformation("Credit Withholding Tax Amount must be negative.");
                    txtDocument.CntrlFocus();
                    return;
                }
            if (whtType == 1)
                if (Convert.ToDouble(txtAdjAmt.Text) <= 0)
                {
                    Messages.ShowInformation("Debit Withholding Tax Amount must be positive.");
                    txtDocument.CntrlFocus();
                    return;
                }
        ...
        {
            //Enable fields
              GridWHT.IsEnabled = true;
           
            //Set the amount_adjusted field as active
            if (this.CurrentBusObj.ObjectData != null)
                if (this.CurrentBusObj.ObjectData.Tables["main"].Rows.Count > 0)
                    (GridWHT.xGrid.Records[GridWHT.ActiveRecord.Index] as DataRecord).Cells["amount_to_adjust"].IsActive = true;
        }
        ...
        public override void Save()
        {
            ...
            if (AmountsOffset())
            {
                ...
                //change to a type = 9 for withholding tax adjustment
              
                string NewDocID = cGlobals.BillService.InsertNewAdjustmentPreamble(txtDocument.Text, 9, cGlobals.UserName.ToLower(), Convert.ToDecimal(sAdjAmt));
